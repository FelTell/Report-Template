...
\subsection{LVGL}
LVGL é uma biblioteca gráfica gratuita e de código aberto, anteriormente conhecida como LittlevGL, desenvolvida em C. A licença MIT (Massachusetts Institute of Technology) permite o uso comercial e não comercial de forma gratuita, desde que a devida atribuição seja dada ao autor original. Existem ferramentas como o SquareLine Studio e o LVGL UI Editor que permitem a criação de interfaces gráficas de forma visual, sem a necessidade de escrever código manualmente; porém, essas ferramentas não são gratuitas para uso comercial~\cite{lvgl-docs}.

\subsection{TouchGFX}
TouchGFX é uma biblioteca gráfica desenvolvida pela STMicroelectronics, projetada para criar interfaces de usuário para produtos com microcontroladores da própria STMicroelectronics~\cite{touchgfx-docs}. O código da biblioteca é fechado e proprietário, o que significa que os desenvolvedores não têm acesso ao código-fonte completo. A interface é desenvolvida em C++. A biblioteca é gratuita para uso comercial, desde que o projeto utilize microcontroladores da STMicroelectronics~\cite{st-touchgfx-license-4-21}. A ferramenta oferece uma interface visual para criar e personalizar interfaces gráficas de forma gratuita.

\subsection{Qt for MCUs}
Qt é um framework amplamente utilizado para o desenvolvimento de aplicações com interfaces gráficas multiplataforma, incluindo desktops, dispositivos móveis e sistemas embarcados. Qt for MCUs (MCU, do inglês Microcontroller Unit) é uma versão do framework Qt projetada especificamente para microcontroladores. A ferramenta tem código aberto e é desenvolvida em C++. A licença do Qt permite uso gratuito apenas para projetos de código aberto; para uso comercial, é necessário adquirir uma licença paga. Existem ferramentas como o Qt Design Studio que permitem a criação de interfaces gráficas de forma visual; porém, essas ferramentas também não são gratuitas para uso comercial~\cite{qt-embedded-docs}.

\subsection{Embedded Wizard}
Embedded Wizard é uma biblioteca gráfica proprietária desenvolvida pela empresa alemã TARA Systems GmbH. O desenvolvimento é feito em C. O código-fonte da ferramenta é fechado e está disponível apenas para clientes que adquirirem uma licença profissional paga. Existe uma opção de licença gratuita, mas com limitações de uso, servindo apenas para fins de avaliação, incluindo uma marca d'água nas interfaces criadas e sem acesso ao código-fonte, apenas à biblioteca pré-compilada. A ferramenta Embedded Wizard Studio oferece uma interface visual para criar e personalizar interfaces gráficas~\cite{embedded-wizard-st-platforms}.

\section{Interfaces}
Para que o microcontrolador transmita as informações para a tela, é necessário utilizar uma interface de comunicação. As interfaces mais comuns em sistemas embarcados incluem I²C, SPI e a interface paralela. 

A interface I²C costuma ser empregada em telas pequenas e de baixa resolução, enquanto a SPI é mais rápida e pode ser usada em telas de resoluções maiores. Ainda assim, ambas podem se tornar gargalos na transferência de dados, especialmente ao lidar com atualizações completas da tela e animações. A comunicação paralela oferece maior velocidade de transferência, porém requer mais pinos de conexão. Ela será empregada neste trabalho para minimizar o tempo de envio de dados entre o microcontrolador e a tela, permitindo avaliar as bibliotecas de GUI com o menor gargalo possível na comunicação.

É importante esclarecer que essas interfaces dizem respeito ao contexto de microcontroladores. Em sistemas embarcados baseados em microprocessadores, existem outras interfaces, como HDMI e DisplayPort, mais comuns em dispositivos com maior capacidade de processamento.

\section{Tecnologia de telas}

As telas usadas em dispositivos embarcados variam significativamente em tamanho, resolução e tecnologia de exibição, indo de pequenas telas LCD monocromáticas a painéis de alta resolução. Neste trabalho será utilizada uma tela LCD-TFT. Há, contudo, outras tecnologias, como IPS, que oferece ângulos de visão mais amplos; OLED, que proporciona melhor contraste e cores mais vibrantes; e E-Ink, que tem taxa de atualização mais lenta, mas consome menos energia e apresenta melhor visibilidade sob luz solar direta. Como o foco deste trabalho é a avaliação das bibliotecas de GUI, a escolha da tecnologia da tela não é o aspecto mais relevante, desde que o display apresente resolução e taxa de atualização adequadas para testar as capacidades dessas bibliotecas.
